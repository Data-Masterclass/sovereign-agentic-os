 ...
 *     helpers (src/scope.ts) filter every record by domain + user (My / Domain).
 ...
 *
 * The Build stage then fills in business features epic-by-epic: each story adds
 * a page under src/pages/ + one SECTIONS entry (src/sections.tsx). README.md
 * documents that contract for the building agent, and the same text is the
 * app's `docs` so the Build assistant reads it as context.
 *
...
function srcAppMetaTs(name: string, slug: string): ScaffoldFile {
  return {
    ...
    content: [
      ...
      " *  '' = same-origin — correct inside the OS instant preview. */",
      'export function osBaseUrl(): string {',
      "  return (import.meta.env.VITE_OS_API as string | undefined) ?? '';",
      '}',
      '',
      '/**',
...
function srcRolesTs(): ScaffoldFile {
  return {
    path: 'src/roles.ts',
    content: [
      '/**',
...
function srcIdentityTsx(): ScaffoldFile {
  return {
    path: 'src/identity.tsx',
    content: [
      ...
      ' * it NEVER invents an anonymous or local user.',
      ' */',
      '',
...
function srcScopeTs(): ScaffoldFile {
  return {
    path: 'src/scope.ts',
    content: [
...
function srcSectionsTsx(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      ' *   1. Create its page:  src/pages/<Name>.tsx  (default-export a component).',
      ...
      ' * src/App.tsx — do not add it here.',
      ' */',
      '',
      ...
      '  // Placeholder — the first built story replaces this with a real section:',
...
function srcAppTsx(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      "import { IdentityProvider, SignedOutScreen, useIdentity } from './identity.tsx';",
      ...
      ' * section. Sections live in src/sections.tsx (epics add entries there); the',
      ' * Admin section is appended here for OS domain admins only.',
      ' */',
      'function Shell() {',
      ...
      '      {/* The owning domain — every read/write is scoped to it (src/scope.ts). */}',
...
function pageOverviewTsx(): ScaffoldFile {
  return {
...
function pageWorkspaceTsx(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      '',
      '/**',
      ' * Placeholder section — the first built story replaces this with a real page.',
      ' * Keep the pattern: one page per section, registered in src/sections.tsx.',
      ' */',
      ...
      '          then build it story by story — the Build assistant adds real pages here.',
      '        </p>',
      '      </Card>',
      '    </Section>',
      '  );',
...
function pageAdminTsx(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      '',
      '/**',
      ' * The Admin section — OS domain_admin / admin only (App.tsx gates the nav; this',
      ' * page also self-guards). User management STAYS in the OS: this list is read-only,',
      ' * fetched from the governed OS directory as the signed-in user.',
      ' */',
...
function decisionsMd(name: string): ScaffoldFile {
  return {
    ...
    content: [
      ...
      '  user; there are no local accounts or passwords (src/identity.tsx).',
      '- The owning domain comes from the deployed host (`<slug>.<domain>.<apps-domain>`,',
      '  src/app-meta.ts); every record is scoped by owner + domain (src/scope.ts).',
...
export function sovereignAppGuide(name: string, slug: string): string {
  return [
    ...
    '| File | Role |',
    '|---|---|',
    '| `src/App.tsx` | Identity gate → `AppShell` chrome → current section. Appends Admin for OS domain admins. |',
    '| `src/sections.tsx` | **The section registry — epics add nav entries here.** One page per section. |',
    '| `src/pages/Overview.tsx` | Starter page: the signed-in OS user + granted context. |',
    '| `src/pages/Workspace.tsx` | Placeholder — replace with the first real feature section. |',
    '| `src/pages/Admin.tsx` | Admin (OS `domain_admin`/`admin` only): settings placeholder + read-only OS user list. |',
    '| `src/identity.tsx` | OS-delegated identity: `os.whoami()`, signed-out screen. **No local auth, ever.** |',
    '| `src/scope.ts` | My / Domain data-scoping helpers — every record carries `owner` + `domain`. |',
    '| `src/roles.ts` | The OS role ladder + `roleAtLeast` (a floor check, never an exact set). |',
    '| `src/app-meta.ts` | App name/slug, OS base URL, owning-domain derivation, MCP setup link. |',
    '| `src/os.ts` | The `@sovereign-os/app-sdk` client factory (governed OS calls). |',
    '',
    '## How epics add features',
    '',
    '1. Create `src/pages/<Feature>.tsx` (default-export a React component; use the',
    '   `@sovereign-os/ui` primitives — `Section`, `Card`, `Table`, `Badge`, `Button`).',
    '2. Register it in `src/sections.tsx` — nav + routing pick it up automatically.',
    '3. Read data ONLY through the OS SDK (`src/os.ts`) — governed, as the signed-in user.',
    '4. Scope every app-local record with `src/scope.ts` (`stamp` on write, `visibleTo` on read).',
    '',
