...
function srcAppTsx(name: string): ScaffoldFile {
  return {
    ...
    content: [
      ...
      '};',
      '',
      'type MetricSample = { id: string; value: number | string; unit?: string } | null;',
      '',
      'const os = createOsClient();',
      '',
      'export default function App() {',
      '  const [user, setUser] = useState<WhoAmI | null>(null);',
      '  const [ctx, setCtx] = useState<ContextGrant | null>(null);',
      '  const [sample, setSample] = useState<MetricSample>(null);',
      ...
      '    (async () => {',
      '      try {',
      '        const [whoami, context] = await Promise.all([os.whoami(), os.context()]);',
      '        if (cancelled) return;',
      '        setUser(whoami as WhoAmI);',
      '        setCtx(context as ContextGrant);',
      '        // Pull one live metric sample if any metrics are granted.',
      '        const metrics = (context as ContextGrant).metrics ?? [];',
      '        if (metrics.length > 0) {',
      '          try {',
      '            const m = await os.queryMetric(metrics[0]);',
      '            if (!cancelled) setSample(m as MetricSample);',
      '          } catch {',
      ...
      '                {String(sample.value)}',
      '                {sample.unit && (',
      '                  <span className="ml-1 text-sm text-neutral-400">{sample.unit}</span>',
      '                )}',
      '              </p>',
      '              <p className="mt-1 text-xs text-neutral-400">{sample.id}</p>',
      '            </CardContent>',
      '          </Card>',
      '        )}',
      '',
      ...
      '                {datasets.map((d) => (',
      '                  <li key={d} className="text-sm text-neutral-700 font-mono">{d}</li>',
      '                ))}',
      '              </ul>',
      '            </CardContent>',
      '          </Card>',
      ...
      '              <ul className="space-y-1">',
      '                {metricRefs.map((m) => (',
      '                  <li key={m} className="text-sm text-neutral-700 font-mono">{m}</li>',
      '                ))}',
      '              </ul>',
      '            </CardContent>',
      '          </Card>',
      ...
      '              <ul className="space-y-1">',
      '                {knowledge.map((k) => (',
      '                  <li key={k} className="text-sm text-neutral-700 font-mono">{k}</li>',
      '                ))}',
      '              </ul>',
      '            </CardContent>',
      '          </Card>',
