...
function dockerfile(): ScaffoldFile {
  return {
    ...
    content: [
      ...
      'WORKDIR /app',
      'COPY . .',
      '# Run as the built-in non-root `node` user (uid 1000) so the OS runner can',
      '# enforce a hardened securityContext (runAsNonRoot, drop ALL caps). Listens on',
      '# 8080 (>1024, no NET_BIND_SERVICE capability needed).',
      'USER node',
