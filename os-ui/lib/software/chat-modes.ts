...
export const CODE_STRUCTURE_CONVENTION = [
  ...
  "  3. `../../../core/store` — the OS client `os` ({ os } — the depth from src/epics/<epic>/<story>/).",
  ...
  'EVENT HANDLERS: `onChange` gives a DOM event, not the value — read',
  '`(e.target as HTMLSelectElement).value` (or HTMLInputElement / HTMLTextAreaElement) inside the handler.',
  '',
  ...
  switch (mode) {
    ...
    case 'build':
      return [
        ...
        '',
        'ROLES & PERMISSIONS — use the scaffolded helpers, never invent a check. Get the user from',
        "`useIdentity()` (src/template/identity.tsx) — when `id.phase === 'ready'` it carries",
        '`id.user.role` (string|null) and `id.user.domains`. Gate with',
