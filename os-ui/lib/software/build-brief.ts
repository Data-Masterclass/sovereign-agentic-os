...
export const OS_SDK_BRIEF = [
  ...
  'the OS registers in `src/template/sections.tsx`. No `<Link>`, no `useNavigate`, no `<Routes>`.',
  '',
  ...
  '',
  'ROLES & PERMISSIONS — get the user from `useIdentity()` (src/template/identity.tsx: phase "ready"',
  'carries user.role + user.domains) and gate with `roleAtLeast(user.role, \'<floor>\')` from',
  'src/template/roles.ts. NEVER an exact match like `role === \'admin\'` — it hard-blocks a real admin',
  '(ladder: creator < builder < domain_admin < admin; "admins/managers can X" ⇒ usually',
  '\'domain_admin\'). Do NOT invent app roles/permissions. Only gate when the role is KNOWN; while',
  'loading or role null, treat as not-yet-known, not denied. Client checks are ADVISORY UX — HIDE or',
  'DISABLE the control (quiet, friendly), never a scary blocking error; the OS data layer enforces.',
  '',
