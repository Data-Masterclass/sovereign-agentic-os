...
**Single engine:** ONE governed query engine — Trino/Iceberg. Your OWN data is a physical Iceberg table in `iceberg.personal_<uid>.*`, read AS you through the SAME governed path (owner-principal) as shared marts; there is no separate personal/DuckDB query engine. Promoted assets/products live in the domain schema.
