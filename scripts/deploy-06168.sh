...
# to compile the ENTIRE schema when one model is invalid (/meta 500 -> every build/promote down).
# Carries all of 0.6.167. See CHANGELOG.md.
set -euo pipefail
cd "$(dirname "$0")/.."
export KUBECONFIG="${KUBECONFIG:-$HOME/.kube/config}"
IMG=ghcr.io/aborek/sovereign-os/os-ui:0.6.168
...
echo "==> helm upgrade (pinned to 0.6.168@$DIGEST; chart 0.2.12 adds the wait-for-opensearch initContainer)"
helm -n agentic-os upgrade agentic-os charts/sovereign-agentic-os \
  --reuse-values --force-conflicts \
  --set osUI.image.tag="0.6.168@$DIGEST" \
